...
let getNotams = function (location, finish) {
    pool.query(
            ...
            "where " +
                ...
                "and (validtotimestamp is null or validtotimestamp AT TIME ZONE 'UTC' > NOW()) " + 
                "and notamtext not like '%NOTAMC%' " + 
                "and status = 'ACTIVE' " +
            "order by validtotimestamp desc, notamid asc",
    [location],
    (err, rows) => {
        if (err) {
...
router.get('/:location', function (req, res) {
    console.log("Notam request for location="+req.params.location+" from "+req.headers['x-forwarded-for']);
